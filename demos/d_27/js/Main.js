        ...
        /** @lends demo.Main.prototype */
        {
           ...
           _handleOverallProgress : function(e)
            ...
            _init:function()
            {
                ...
                //从游戏场景中获取quad
                //resister quad
                this.actor=quadlist.applyQuad();

                //设置quad偏移,在纹理上设置到正确的切片位置
                //set quad offset， so we can get the right clip（32*32） of the cartoon man from the texture
                this.actor.setTileWidthOffset(-this.actor.getWidth()+32);
                this.actor.setTileHeightOffset(-this.actor.getHeight()+32);

                //提交给场景显示
                //display quad
                quadlist.showQuad(this.actor);
            ...
            //基于切片的动画处理 (设置切片偏移实现)
            //clip-based animation（set quad offset per cycle）
            gap:0,
            time:0,
            orientation:1,
            onEnterFrameHandler : function(e)
            {
                ...
                this.actor.setTileXOffset(this.time*32);
            }
